...
func (nq *NodeQuery) loadPrimaryPurpose(ctx context.Context, query *PurposeQuery, nodes []*Node, init func(*Node), assign func(*Node, *Purpose)) error {
	...
	for _, n := range neighbors {
		fk := n.node_primary_purpose
		if fk == nil {
			return fmt.Errorf(`foreign-key "node_primary_purpose" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "node_primary_purpose" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
